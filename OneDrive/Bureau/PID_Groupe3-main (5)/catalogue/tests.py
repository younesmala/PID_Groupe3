...
from catalogue.models import Price, Representation, Reservation, Show, UserMeta
...
class CartTestCase(TestCase):
    ...
    def test_cart_update_quantity(self):
        ...
        response = self.client.post(
            reverse("cart:cart_add", args=[self.representation.id]),
            {"price_id": self.price.id, "quantity": 5, "override_quantity": "true"},
        )
        self.assertEqual(response.status_code, 302)
        session = self.client.session
        key = f"{self.representation.id}_{self.price.id}"
    ...
    def test_cart_remove_item(self):
        ...
        response = self.client.post(
            reverse("cart:cart_remove", args=[self.representation.id, self.price.id])
        )
        self.assertEqual(response.status_code, 302)
        session = self.client.session
