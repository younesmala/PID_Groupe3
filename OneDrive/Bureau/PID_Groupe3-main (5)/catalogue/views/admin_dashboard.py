...
class AdminDashboardView(View):
    ...
    def get(self, request):
        reservations_qs = Reservation.objects.select_related("user").order_by(
            "-booking_date"
        )
        recent_users = User.objects.prefetch_related("groups").order_by("-date_joined")[
            :5
        ]
...
class AdminUserEditView(View):
    ...
    def get(self, request, pk):
        ...
        current_group = target.groups.filter(
            name__in=["MEMBER", "ADMIN", "PRODUCER"]
        ).first()
        return render(
            request,
