...
from catalogue.models import Price, Representation
...
class CartView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        return Response(
            {"detail": "Use /api/cart/add/"}, status=status.HTTP_400_BAD_REQUEST
        )
...
class CartAddView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if quantity is None or quantity <= 0:
            return Response(
                {"detail": "Quantité invalide."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if price_id:
            ...
            if not price:
                return Response(
                    {"detail": "Tarif introuvable."}, status=status.HTTP_404_NOT_FOUND
                )
        else:
            return Response(
                {"detail": "ID de prix ou type de prix requis."},
