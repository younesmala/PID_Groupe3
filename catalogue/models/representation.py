...
class RepresentationManager(models.Manager):
    def get_by_natural_key(self, show_slug, show_created_in, location_slug, location_website, schedule):
        ...
        location = Location.objects.get_by_natural_key(
            location_slug, location_website)
        return self.get(show_id=show.pk, location_id=location.pk, schedule=schedule)


class Representation(models.Model):
    show = models.ForeignKey(
        Show, on_delete=models.RESTRICT, null=False, related_name='representations')
    schedule = models.DateTimeField()
    location = models.ForeignKey(
        Location, on_delete=models.RESTRICT, null=True, related_name='representations')
    available_seats = models.PositiveIntegerField(
        default=100)  # Nombre de places disponibles
